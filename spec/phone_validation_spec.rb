...
RSpec.describe PhoneValidation::Client do
  ...
  subject { described_class.new(token: token, phone_number: phone_number) }
  ...
  context 'instance methods' do
    ...
    before do
      expect(PhoneValidation::ValidationRequest).
        to receive(:new).
        once.
        with(token: token, phone_number: phone_number).
    ...
    describe '#valid?' do
      subject { described_class.new(token: token, phone_number: phone_number).valid? }
    ...
    describe '#local_format' do
      subject { described_class.new(token: token, phone_number: phone_number).local_format }

      it_behaves_like 'returns method name field extracted from response', '977222122'
    end

    describe '#international_format' do
      subject { described_class.new(token: token, phone_number: phone_number).international_format }

      it_behaves_like 'returns method name field extracted from response', '+34977222122'
    end

    describe '#country_prefix' do
      subject { described_class.new(token: token, phone_number: phone_number).country_prefix }

      it_behaves_like 'returns method name field extracted from response', '+34'
    end

    describe '#country_code' do
      subject { described_class.new(token: token, phone_number: phone_number).country_code }

      it_behaves_like 'returns method name field extracted from response', 'ES'
    end

    describe '#country_name' do
      subject { described_class.new(token: token, phone_number: phone_number).country_name }

      it_behaves_like 'returns method name field extracted from response', 'Spain'
    end

    describe '#location' do
      subject { described_class.new(token: token, phone_number: phone_number).location }

      it_behaves_like 'returns method name field extracted from response', 'Tarragona'
    end

    describe '#carrier' do
      subject { described_class.new(token: token, phone_number: phone_number).carrier }

      it_behaves_like 'returns method name field extracted from response', ''
    end

    describe '#line_type' do
      subject { described_class.new(token: token, phone_number: phone_number).line_type }
